...
한 커피 제조회사에서는 새로 출시한 커피의 카페인 함량이 평균 95mg 미만이라고 주장하였다. 그 주장이 사실인가를 알아보기 위하여 25개의 커피 샘플을 무작위(랜덤)로 추출하였다. 커피 제조회사의 주장이 타당한지를 유의수준 5%에서 검정하시오.

- 귀무가설(H0): μ ≥ 95mg
- 대립가설(H1): μ < 95mg
1. 표본 데이터의 평균을 구하시오.
2. Shapiro-Wilk의 p-value를 구하시오.
3. 단일 표본 t-검정의 검정통계량을 구하시오.
4. 단일 표본 t-검정의p-value를 구하시오.
5. 유의 수준 0.05하에서 귀무가설을 기준으로 검정의 결과를 (채택/기각)중 선택하여 입력하시오.

mean_val = df['Caffeine(mg)'].mean()
print(f"표본 평균: {mean_val}") # 결과: 94.264

from scipy import stats
shapiro_pval = stats.shapiro(df['Caffeine(mg)'])[1]
print(f"Shapiro P-value: {shapiro_pval}") # 결과: 0.9322 (정규성 만족)

t_stat, p_val = stats.ttest_1samp(df['Caffeine(mg)'], 95, alternative='less')

print(f"검정통계량: {t_stat}") # 결과: -5.5017
print(f"P-value: {p_val:.10f}") # 결과: 0.0000058687

# **2 독립 표본 검정**

스마트폰 배터리의 충전 시간을 줄이기 위한 새로운 충전기를 개발하였다. 개발된 충전기의 효과를 검정하기 위해 스마트폰 사용자 집단을 두 그룹으로 나누어 한 그룹에는 새로운 충전기를 제공하고, 나머지 그룹에는 기존의 충전기를 제공하였다. 그 후, 두 그룹의 평균 충전 완료 시간을 비교하였다. 두 그룹 모두 충전 시간은 정규분포를 따르며 분산은 같다고 가정한다. 새로운 충전기가 더 빠르다고 할 수 있는지를 유의수준 5%하에서 검정하시오. 여기서 두 집단은 모두 정규분포를 따르고 분산은 같다고 가정한다. (μ1: 새로운 충전기 집단의 모평균, μ2: 기존 충전기 집단의 모평균)

- 귀무가설(H0): μ1 ≥ μ2
- 대립가설(H1): μ1 < μ2
1. 위의 가설을 검정하기 위한 검정 통계량을 구하시오.
2. 위의 통계량에 대한 p-value를 구하시오.
3. 유의 수준 0.05하에서 귀무가설을 기준으로 검정의 결과를 (채택/기각)중 선택하여 입력하시오.

from scipy import stats

df_new = df[df['충전기'] == "New"]['충전시간']
df_old = df[df['충전기'] == "Old"]['충전시간']s)

t_stat, p_val = stats.ttest_ind(df_new, df_old, alternative='less', equal_var=True)

print(f"검정통계량: {t_stat}") # 결과: -4.5825
print(f"P-value: {p_val}") # 결과: 0.000115

# **3 대응 표본 검정**

딴짓 연구소에서는 새로 개발한 교육과정에 참여 하면 학습 시간이 단축한다고 주장한다. 10명의 학생들을 대상으로 기존의 교육 방법과 새로운 교육 방법을 적용한 결과에 대하여 유의수준 5%에서 새로운 교육 방법이 교육 시간을 단축시켰는지 검정하시오. (단, 모집단은 정규분포를 가정한다.)

- μd = (새로운 교수 방법 – 기존 교수 방법)의 평균
- 귀무가설(H0): μd = 0
- 대립가설(H1): μd < 0
1. μd의 표본평균을 구하시오.
...
diff_mean = (df['새로운방법'] - df['기존방법']).mean()
print(f"차이 평균: {diff_mean}") # 결과: -1.03

t_stat, p_val = stats.ttest_rel(df['새로운방법'], df['기존방법'], alternative='less')

print(f"검정통계량: {t_stat}") # 결과: -3.4079
print(f"P-value: {p_val}") # 결과: 0.00388

# **4. 일원 분산 분석**

4개의 중학교에서 다른 교육 방법을 사용하여 학생들에게 수학을 가르쳤습니다. 각 중학교의 학생들이 받은 교육 후의 수학 성적은 다음과 같습니다. 이 4개의 교육 방법에 따른 성적 차이가 통계적으로 유의미한지 5%의 유의 수준에서 검정하십시오.

- 귀무가설(H0): 모든 중학교의 학생들의 수학 성적 평균은 동일하다.
- 대립가설(H1): 적어도 두 개의 중학교의 학생들의 수학 성적 평균은 다르다. (4개 교육 방법 중 최소한 하나의 방법이 다른 방법들과 성적 차이가 있다.)
1. 각 그룹의 수학 성적에 대해 Shapiro-Wilk 검정을 통한 정규성을 확인하고, 그 결과의 p-value를 구하시오
2. 4개 그룹의 수학 성적이 등분산성을 가지는지 확인하기 위해 Levene 검정을 실시하고 p-value를 구하시오.
3. 유의 수준 0.05하에서 귀무가설을 기준으로 검정의 결과를 (채택/기각)중 선택하여 입력하시오.
4. 그룹 변수의 자유도를 구하시오.
5. 잔차의 자유도를 구하시오.
6. 성적의 제곱합을 구하시오.
...
for col in ['A', 'B', 'C', 'D']:
print(f"{col} 정규성: ", stats.shapiro(df[col])[1])

print("등분산성: ", stats.levene(df['A'], df['B'], df['C'], df['D'])[1])

f_stat, p_val = stats.f_oneway(df['A'], df['B'], df['C'], df['D'])
print(f"F-통계량: {f_stat}, P-value: {p_val}")
...
H 토마토 연구소에서는 토마토의 수확량을 향상시키기 위해 3가지 다른 비료 유형과 4가지 다른 물 주기를 실험하고자 한다. 연구소에서는 12개의 토마토 식물 무작위로 선택하여 각 조합에 대해 반복적으로 실험을 수행하였다.

비료 유형에 따른 효과

- 귀무가설(H0): 모든 비료 유형의 토마토 수확량 평균은 동일하다.
- 대립가설(H1): 적어도 두 개 이상의 비료 유형에서의 토마토 수확량 평균이 다르다.

물 주기에 따른 효과

- 귀무가설(H0): 모든 물 주기의 토마토 수확량 평균은 동일하다.
- 대립가설(H1): 적어도 두 개 이상의 물 주기에서의 토마토 수확량 평균이 다르다.

비료 유형과 물 주기간의 상호작용 효과

- 귀무가설(H0): 비료 유형과 물 주기 간의 상호작용은 토마토 수확량에 영향을 미치지 않는다.
- 대립가설(H1): 비료 유형과 물 주기 간의 상호작용은 토마토 수확량에 영향을 미친다.
1. 비료 유형에 따른 토마토의 수확량 평균에 차이가 있는가를 검정하기 위한 검정 통계량을 구하시오.
2. 위의 통계량에 대한 p-value를 구하시오.
3. 위의 검정 결과를 유의 수준 0.05하에서 귀무가설을 기준으로 (채택/기각)중 선택하여 입력하시오.
4. 물 주기에 따른 토마토의 수확량 평균에 차이가 있는가를 검정하기 위한 검정 통계량을 구하시오.
5. 위의 통계량에 대한 p-value를 구하시오.
6. 위의 검정 결과를 유의 수준 0.05하에서 귀무가설을 기준으로 (채택/기각)중 선택하여 입력하시오.
7. 비료 유형과 물 주기 간의 상호작용은 토마토 수확량에 영향이 있는지 검정하기 위한 검정 통계량을 구하시오.
8. 위의 통계량에 대한 p-value를 구하시오.
9. 위의 검정 결과를 유의 수준 0.05하에서 귀무가설을 기준으로 (채택/기각)중 선택하여 입력하시오.

from statsmodels.formula.api import ols
from statsmodels.stats.anova import anova_lm

model = ols('토마토수 ~ C(비료) * C(물주기)', data=df).fit()

print(anova_lm(model))

해석:

PR(>F) 값이 0.05보다 작으면 해당 요인(또는 상호작용)이 유의하다고 판단하여 귀무가설 기각.

# **6. 적합도 검정**

한 도시의 운전자 1,000명을 대상으로 교통사고 경험 수를 조사하였다. 1회: 550명, 2회: 250명, 3회 100명, 4회 70명, 5회 이상: 30명이다. 전국적으로 조사된 데이터에 따르면, 운전자들의 교통사고 경험 수 분포는 다음과 같다. 1회: 60%, 2회: 25%, 3회 8%, 4회 5%, 5회 이상: 2%이다. 이 도시 운전자들의 교통사고 경험 수 분포가 전국적인 경향을 따르는지 검정하시오. (유의수준 0.05)

- **귀무가설**(H0): 이 도시의 교통사고 경험 수 분포는 전국적인 경향을 따른다.
- **대립가설**(H1): 이 도시의 교통사고 경험 수 분포는 전국적인 경향을 따르지 않는다.

![image.png](https://cdn.inflearn.com/public/files/posts/308cb51d-24e5-48ee-ba46-1a9420f1a773/9c2ab596-f268-439a-9681-811229058508.png)

1. 이 도시의 교통사고 경험자 중 5회 이상의 비율을 0과 1사이로 구하시오.
2. 이 도시의 운전자들의 교통사고 경험 수 분포가 전국적인 경향을 따르는지를 검정하기 위한 검정 통계량을 구하시오.
3. 위의 통계량에 대한 p-value를 구하시오.
4. 위의 검정 결과를 유의 수준 0.05하에서 귀무가설을 기준으로 (채택/기각)중 선택하여 입력하시오.

from scipy import stats

observed = [550, 250, 100, 70, 30]

total = sum(observed) # 1000
expected = [total*0.6, total*0.25, total*0.08, total*0.05, total*0.02]

chi_stat, p_val = stats.chisquare(f_obs=observed, f_exp=expected)

print(f"검정통계량: {chi_stat}")
print(f"P-value: {p_val}") # 결과: 0.000185

# **7. 독립성 검정**

딴짓 대학에는 빅데이터 분석기사 캠프과 정보처리기사 캠프가 있다. 최근 양 캠프 학생들 사이에서 주최한 세미나에 참여하려면 등록을 해야 한다. 각 캠프의 세미나 등록여부를 조사한 결과는 아래와 같다. 캠프와 세미나 등록 여부는 서로 독립인지 검정하시오. (유의수준 0.05)

- 귀무가설(H0): 캠프와 세미나 등록 여부는 서로 독립니다.
- 대립가설(H1): 캠프와 세미나 등록 여부는 서로 독립이 아니다.

A. 교차표 데이터가 주어졌을 때

![image.png](https://cdn.inflearn.com/public/files/posts/2b498b66-9783-4836-b04b-c89f33351a63/69aaf3e8-3b17-4260-a6c0-3c3470640f31.png)

B. **csv 파일(raw) 데이터가 주어졌을 때**

1. 캠프와 세미나 등록여부가 서로 독립인지를 검정하기 위한 검정 통계량을 구하시오.
2. 위의 통계량에 대한 p-value를 구하시오.
3. 위의 검정 결과를 유의 수준 0.05하에서 귀무가설을 기준으로 (채택/기각)중 선택하여 입력하시오.
...
data = pd.DataFrame([[50, 30], [60, 40]])

chi_stat, p_val, dof, expected = chi2_contingency(data)

print(f"검정통계량: {chi_stat}") # 결과: 0.035
print(f"P-value: {p_val}") # 결과: 0.8508
...
1. 할인율와 온도의 상관계수를 구하시오. (반올림하여 소수 둘째자리까지 계산)
...
2. 모델의 결정계수를 구하시오. (반올림하여 소수 둘째자리까지 계산)
3. 각 변수의 회귀 계수를 구하시오. (반올림하여 소수 넷째자리까지 계산)
4. 모델의 절편 (Intercept)을 구하시오. (반올림하여 소수 넷째자리까지 계산)
5. 온도의 회귀 계수가 통계적으로 유의한지 검정하시오. (p-value값을 반올림하여 소수 넷째자리까지 계산)
...
```
new = pd.DataFrame({'할인율' : [10], '온도' : [20], '광고비' : [500]})
...
4. 데이터가 할인율이15%, 온도가 25°C, 광고비가 300,000원일 때 90%의 신뢰구간과 예측구간을 구하시오.
    
    ```
    new2 = pd.DataFrame({"할인율": [15], "온도": [25], "광고비": [300]})
...
5. 독립변수 할인율과 온도를 고정한 상태에서, 광고비가 배달 주문량에 영향을 주는지 가설을 검정하시오. (유의수준 0.05) 귀무가설 기각 또는 채택으로 답하시오.
- 귀무가설(H0): 광고비의 회귀 계수는 0이다. (광고비가 배달 주문량에 영향을 주지 않는다.)
- 대립가설(H1): 광고비의 회귀 계수는 0이 아니다. (광고비가 배달 주문량에 영향을 준다.)
    
    # 광고비 p-value : 0.003 -> 귀무가설 기각
...
3. 수정된 모델에서 독립변수 중 절댓값이 가장 큰 회귀계수를 가진 변수의 이름을 구하시오.
...
8. 수정된 모델로 b 데이터를 사용해 예측 후, b 데이터의 target과 비교해 정확도(Accuracy)를 계산하시오. 정확도는 0과 1 사이의 값이다.
...
insignificant_vars = (model.pvalues[1:] >= 0.05).sum()
print(f"1. 유의하지 않은 변수 개수: {insignificant_vars}")

max_pval_var = model_new.pvalues.idxmax()
print(f"2. 가장 큰 p-value 변수: {max_pval_var}")

max_coef_var = model_new.params.abs().idxmax()
print(f"3. 절댓값 최대 회귀계수 변수: {max_coef_var}")

print(f"4. 로그 우도: {model_new.llf:.4f}")

deviance = -2 * model_new.llf
print(f"5. 잔차 이탈도: {deviance:.4f}")

odds_ratio = np.exp(model_new.params['booked'] * 3)
print(f"6. booked 3증가 시 오즈비: {odds_ratio:.4f}")

sum_sig_coefs = model_new.params[model_new.pvalues < 0.05].sum()
print(f"7. 유의한 회귀계수 합: {sum_sig_coefs:.4f}")

pred_prob = model_new.predict(df_b)
pred_class = (pred_prob > 0.5).astype(int)
accuracy = (pred_class == df_b['target']).mean()
print(f"8. 정확도: {accuracy:.4f}")

error_rate = 1 - accuracy
print(f"9. 오류율: {error_rate:.4f}")
